...
import {
  Menu, X, Play, Volume2, Scale, CalendarDays, ShieldCheck, Leaf, Truck, Syringe,
  ChevronRight, ChevronLeft, ArrowRight, Calculator as CalcIcon, Search, MapPin, Phone,
  Mail, Clock, Plus, Minus, Check, Star, Heart, Award, Sparkles, ChevronDown,
  LayoutDashboard, Package, FileText, Tag, ClipboardList, Settings, Bell, Pencil, Trash2,
  Copy, Download, LogOut, ExternalLink, Image as ImageIcon, Video, ArrowUp, ArrowDown,
  Save, RotateCcw, CheckCircle2, AlertCircle, TrendingUp, Wallet, Upload, Store, Eye
} from "lucide-react";
import { PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, ResponsiveContainer, Tooltip } from "recharts";

/* ===================== HELPER & KONSTANTA ===================== */
const STATUS = { tersedia: { label: "Tersedia", cls: "st-avail" }, booked: { label: "Booked", cls: "st-booked" }, terjual: { label: "Terjual", cls: "st-sold" } };
const TINTS = ["#6F8B7C", "#7A8B6F", "#8B7F6F", "#6F8589", "#7C8267", "#6F7E8B", "#86827A"];
const ICONS = { shield: ShieldCheck, scale: Scale, truck: Truck, leaf: Leaf, syringe: Syringe, heart: Heart, award: Award, clock: Clock };
const DP_PCT = 0.3;
...
const idrShort = (n) => { n = Number(n) || 0; return n >= 1e6 ? "Rp " + (n / 1e6).toFixed(n % 1e6 === 0 ? 0 : 1).replace(".", ",") + " jt" : idr(n); };
const fmtDate = (iso) => new Date(iso).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" });
const fmtTime = (iso) => new Date(iso).toLocaleString("id-ID", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" });
const est = (s) => (Number(s.hargaPerKg) || 0) * (Number(s.bobot) || 0);

const waInquiry = (st, s) => {
  const url = `${st.siteUrl}/domba/${s.id}`;
  const t = `Halo ${st.brand}, saya tertarik dengan domba *${s.id}* (${s.jenis}).\n\nBobot ${s.bobot} kg × ${idr(s.hargaPerKg)}/kg ≈ ${idr(est(s))}.\nApakah masih tersedia?\n\nLink: ${url}`;
  return `https://wa.me/${st.waNumber}?text=${encodeURIComponent(t)}`;
};
const waCalc = (st, { sheep, weight, rate, qty, total }) => {
  const L = [`Halo ${st.brand}, saya mau pesan domba dengan estimasi berikut:`, ""];
  ...
  return `https://wa.me/${st.waNumber}?text=${encodeURIComponent(L.join("\n"))}`;
};

/* ===================== DATA AWAL ===================== */
function makeSeed() {
  const base = [
    ["DMB-001", "Domba Garut", 42, 14, "tersedia", 110000, "Hitam–Putih", true, "Garut jantan postur tegap, leher tebal. Cocok untuk kontes maupun kurban premium.", "Rumput gajah + konsentrat, 2× sehari", "SE & cacing — lengkap"],
    ["DMB-002", "Dorper", 55, 18, "tersedia", 115000, "Putih–Kepala Hitam", true, "Dorper bobot besar, pertumbuhan cepat, daging padat. Temperamen tenang.", "Hijauan + pakan fermentasi", "SE & cacing — lengkap"],
    ["DMB-003", "Ekor Gemuk (DEG)", 38, 12, "booked", 95000, "Putih", false, "DEG lokal adaptif & tahan cuaca. Pilihan ekonomis kondisi sehat terjaga.", "Rumput lapangan + ampas tahu", "Cacing — lengkap"],
    ["DMB-004", "Merino Cross", 47, 16, "tersedia", 112000, "Krem Wol Tebal", false, "Persilangan Merino, bulu wol tebal & rapat. Ideal untuk display.", "Hijauan + konsentrat tinggi protein", "SE & cacing — lengkap"],
    ["DMB-005", "Texel", 51, 17, "terjual", 118000, "Putih", false, "Texel berotot, proporsi karkas baik. Sudah terjual.", "Hijauan + konsentrat", "SE & cacing — lengkap"],
    ["DMB-006", "Domba Garut", 40, 13, "tersedia", 100000, "Coklat–Putih", false, "Garut muda, tanduk mulai melingkar, gerak lincah.", "Rumput gajah + konsentrat", "Cacing — lengkap"],
    ["DMB-007", "Domba Batur", 49, 15, "booked", 108000, "Putih Wol Tebal", false, "Batur dataran tinggi, wol tebal tahan dingin. Menunggu pelunasan.", "Hijauan dataran tinggi + konsentrat", "SE & cacing — lengkap"],
  ];
  const sheep = base.map((b, i) => ({
    _k: uid(), id: b[0], jenis: b[1], bobot: b[2], umur: b[3], status: b[4], hargaPerKg: b[5], warna: b[6], featured: b[7],
    desc: b[8], pakan: b[9], vaksin: b[10], tint: TINTS[i % TINTS.length],
    media: [{ id: uid("m_"), type: "foto", name: "foto-utama.jpg", url: null, cover: true }],
    log: [{ id: uid("l_"), t: "12 Mei", x: "Penimbangan rutin" }, { id: uid("l_"), t: "28 Apr", x: "Vaksin SE" }],
    createdAt: now(), updatedAt: now(),
  }));
  return {
    sheep,
    breeds: ["Domba Garut", "Dorper", "Ekor Gemuk (DEG)", "Merino Cross", "Texel", "Domba Batur"],
    content: {
      hero: { eyebrow: "Peternakan Domba Premium · Sejak 2015", title: "Domba sehat, terawat,\ndibeli per kilogram.", sub: "Harga transparan per kg dan bobot tertimbang. Pilih dari katalog, hitung estimasi, lalu konfirmasi cepat lewat WhatsApp.", ctaLabel: "Lihat Katalog" },
      badges: [{ ic: "shield", text: "Sehat & bervaksin" }, { ic: "scale", text: "Ditimbang transparan" }, { ic: "truck", text: "Antar ke lokasi" }],
      about: { title: "Tentang Lembah Hijau", body: "Peternakan domba di Garut yang fokus pada kualitas dan kesehatan ternak. Setiap domba ditimbang, diperiksa dokter hewan, dan punya riwayat perawatan yang tercatat. Harga dihitung adil berdasarkan bobot hidup per kilogram." },
    },
    settings: { brand: "Lembah Hijau", tagline: "Peternakan Domba Premium", waNumber: "6281234567890", siteUrl: "https://lembahhijau.id", location: "Garut, Jawa Barat", email: "halo@lembahhijau.id", hours: "Setiap hari, 07.00–17.00", accent: "#059669" },
    log: [{ id: uid(), at: now(), who: "Admin", action: "Data contoh dimuat", kind: "create" }],
  };
}
...
function useDB() {
  ...
  useEffect(() => {
    (async () => {
      if (typeof window === "undefined" || !window.storage) { setPersist(false); setDb(makeSeed()); return; }
      try { const r = await window.storage.get("etalase:db:v2"); if (r && r.value) { setDb(JSON.parse(r.value)); return; } } catch (e) {}
      const seed = makeSeed(); setDb(seed);
      try { await window.storage.set("etalase:db:v2", JSON.stringify(seed)); } catch (e) { setPersist(false); }
    })();
  }, []);
  const commit = useCallback((next) => { setDb(next); if (window.storage) window.storage.set("etalase:db:v2", JSON.stringify(next)).catch(() => setPersist(false)); }, []);
  return { db, commit, persist };
}

/* ===================== KOMPONEN DASAR ===================== */
const SheepThumb = ({ tint = "#6F8B7C", url, size = 44, radius = 9, kind = "foto", playing = false }) => {
  if (url) return kind === "klip"
    ? <video src={url} muted loop playsInline autoPlay style={{ width: size, height: size, borderRadius: radius, objectFit: "cover" }} />
    : <img src={url} alt="" style={{ width: size, height: size, borderRadius: radius, objectFit: "cover" }} />;
  return (
    <div className={"thumb-ph" + (playing ? " play" : "")} style={{ width: size, height: size, borderRadius: radius, "--t": tint }}>
      <svg viewBox="0 0 220 170" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
        {[78, 96, 128, 146].map((x, i) => <rect key={i} x={x} y={104} width={9} height={30} rx={4} fill={tint} opacity={i < 2 ? .95 : .8} />)}
        <ellipse cx={110} cy={86} rx={58} ry={34} fill="#fff" opacity={.92} /><ellipse cx={110} cy={86} rx={58} ry={34} fill={tint} opacity={.32} />
        {[64, 84, 106, 128, 150].map((x, i) => <circle key={i} cx={x} cy={58 + (i % 2) * 4} r={13} fill="#fff" opacity={.92} />)}
        {[64, 84, 106, 128, 150].map((x, i) => <circle key={"t" + i} cx={x} cy={58 + (i % 2) * 4} r={13} fill={tint} opacity={.22} />)}
        <ellipse cx={165} cy={playing ? 96 : 78} rx={17} ry={21} fill={tint} /><ellipse cx={154} cy={playing ? 84 : 66} rx={6} ry={9} fill={tint} opacity={.7} />
        <circle cx={168} cy={playing ? 92 : 74} r={2.4} fill="#1b2a22" />
      </svg>
    </div>
  );
};
const Pill = ({ status, float }) => <span className={"pill " + STATUS[status].cls + (float ? " float" : "")}>{STATUS[status].label}</span>;
const Field = ({ label, hint, required, children }) => (
  <label className="field"><span className="field-label">{label}{required && <em>*</em>}</span>{children}{hint && <span className="field-hint">{hint}</span>}</label>
);
const Toggle = ({ on, onChange, label }) => (
  <button type="button" className={"toggle" + (on ? " is-on" : "")} onClick={() => onChange(!on)}><span className="toggle-dot" /><span>{label}</span></button>
);
...
function Calculator({ db, lockSheep = null, showCta = true, onResult }) {
  ...
  const [key, setKey] = useState(lockSheep ? lockSheep._k : (avail[0]?._k || "manual"));
  const picked = lockSheep || (key !== "manual" ? db.sheep.find(s => s._k === key) : null);
  ...
  useEffect(() => {
    ...
    if (s) { setRate(s.hargaPerKg); setWeight(s.bobot); }
  }, [key]); // eslint-disable-line

  const total = Math.round(rate * weight * qty);
  const dp = Math.round(total * DP_PCT);
  useEffect(() => { onResult && onResult({ weight, rate, qty, total, dp, sheep: picked || null }); }, [weight, rate, qty, total]); // eslint-disable-line
    ...
    <div className="calc">
        ...
        {!lockSheep && (
          <Field label="Pilih domba">
            <select value={key} onChange={e => setKey(e.target.value)}>
              {avail.map(s => <option key={s._k} value={s._k}>{s.id} — {s.jenis} ({s.bobot} kg)</option>)}
              <option value="manual">— Masukkan manual —</option>
            </select>
          </Field>
        )}

        <Field label="Harga per kg" hint={lockSheep ? "Sesuai daftar untuk domba ini" : (key === "manual" ? "Masukkan harga Anda" : "Otomatis dari daftar, bisa diubah")}>
          {lockSheep
            ? <div className="calc-fixed">{idr(rate)}<small>/kg</small></div>
            : <div className="calc-input"><span>Rp</span><input type="number" value={rate} onChange={e => setRate(Number(e.target.value) || 0)} /><b>/kg</b></div>}
        </Field>

        <Field label={`Berat (kg) — ${weight} kg`}>
          <input className="slider" type="range" min={wmin} max={wmax} step={1} value={weight} onChange={e => setWeight(Number(e.target.value))} />
          <div className="calc-input mt"><input type="number" value={weight} onChange={e => setWeight(clampW(e.target.value))} /><b>kg</b></div>
        </Field>
        ...
        {!lockSheep && (
          <Field label="Jumlah ekor">
            <div className="stepper">
              ...
              <span>{qty}</span>
              <button onClick={() => setQty(q => Math.min(99, q + 1))}><Plus size={15} /></button>
            </div>
          </Field>
        )}
      </div>
      ...
      <div className="calc-result">
        ...
        <div className="calc-break">
          <span>{idr(rate)}/kg × {weight} kg{qty > 1 ? ` × ${qty}` : ""}</span>
        </div>
        <div className="calc-dp"><Wallet size={14} /> Estimasi DP {Math.round(DP_PCT * 100)}% · <b>{idr(dp)}</b></div>
        <p className="calc-note"><AlertCircle size={13} /> Berat final ditimbang ulang saat serah terima.</p>
        {showCta && (
          <a className="btn btn-primary btn-block" href={waCalc(st, { sheep: picked, weight, rate, qty, total })} target="_blank" rel="noopener noreferrer">
...
function PriceTag({ s, big }) {
  return (
    <div className={"price" + (big ? " big" : "")}>
      <span className="rate">{idr(s.hargaPerKg)}<small>/kg</small></span>
      <span className="est">≈ {idrShort(est(s))} · {s.bobot} kg</span>
    </div>
  );
}
...
function ProductCard({ s, onOpen, delay }) {
  ...
  return (
    <article className="card-prod rise" style={{ animationDelay: delay + "ms" }} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onClick={() => onOpen(s)}>
      <div className="prod-media">
        <SheepThumb tint={s.tint} url={cover?.url} kind={cover?.type} size={"100%"} radius={0} playing={hover && !cover?.url} />
        <Pill status={s.status} float />
        <span className="clip"><span className="clip-play"><Play size={10} fill="currentColor" /></span>{hover ? "Memutar klip" : "Klip · 4 dtk"}<Volume2 size={11} /></span>
      </div>
      <div className="prod-body">
        <div className="prod-top"><span className="mono">{s.id}</span><span className="muted sm">{s.warna}</span></div>
        <h3>{s.jenis}{s.featured && <Star size={13} fill="#B97309" color="#B97309" />}</h3>
        <div className="prod-meta"><span><Scale size={14} /> {s.bobot} kg</span><span><CalendarDays size={14} /> {s.umur} bln</span></div>
        <div className="prod-foot"><PriceTag s={s} /><span className="detail-link">Detail <ChevronRight size={15} /></span></div>
      </div>
    </article>
  );
}
...
function ProductDetail({ s, db, onClose }) {
  ...
  const [res, setRes] = useState({ weight: s.bobot, rate: s.hargaPerKg, qty: 1, total: est(s), sheep: s });
  const media = useMemo(() => [{ kind: "foto", label: "Foto utama" }, { kind: "foto", label: "Tampak samping" }, { kind: "klip", label: "Klip video" }, { kind: "foto", label: "Tampak depan" }], []);
    ...
    <div className="overlay" role="dialog" aria-modal="true">
      ...
      <div className="sheet">
        ...
        <div className="detail-grid">
          <div className="gallery">
            <div className="gallery-main">
              <SheepThumb tint={s.tint} url={media[active].kind === "klip" ? null : cover?.url} kind={media[active].kind} size={"100%"} radius={0} playing={media[active].kind === "klip"} />
              <Pill status={s.status} float />
              <button className="nav nav-l" onClick={() => go(-1)}><ChevronLeft size={18} /></button>
              <button className="nav nav-r" onClick={() => go(1)}><ChevronRight size={18} /></button>
            </div>
            <div className="thumbs">{media.map((m, i) => (
              <button key={i} className={"thumb" + (i === active ? " on" : "")} onClick={() => setActive(i)}>
                <SheepThumb tint={s.tint} url={null} size={"100%"} radius={0} />
                {m.kind === "klip" && <span className="thumb-play"><Play size={12} fill="currentColor" /></span>}
              </button>
            ))}</div>
          </div>

          <div className="detail-info">
            <span className="eyebrow">{s.id} · {s.warna}</span>
            <h2>{s.jenis}</h2>
            <PriceTag s={s} big />
            <p className="detail-desc">{s.desc}</p>
            <div className="spec">
              <div className="spec-cell"><span>Bobot</span><strong>{s.bobot} kg</strong></div>
              <div className="spec-cell"><span>Umur</span><strong>{s.umur} bulan</strong></div>
              <div className="spec-cell"><span>Harga/kg</span><strong>{idr(s.hargaPerKg)}</strong></div>
              <div className="spec-cell"><span>Status</span><strong className={"sp-st " + STATUS[s.status].cls}>{STATUS[s.status].label}</strong></div>
            </div>
            <div className="care">
              <div className="care-row"><Leaf size={16} /><div><span>Pakan</span><p>{s.pakan}</p></div></div>
              <div className="care-row"><Syringe size={16} /><div><span>Vaksin & kesehatan</span><p>{s.vaksin}</p></div></div>
            </div>
            <div className="timeline">
              <span className="tl-head"><Clock size={14} /> Riwayat perawatan</span>
              {s.log.map(l => <div key={l.id} className="tl-item"><span className="tl-dot" /><b>{l.t}</b> {l.x}</div>)}
            </div>

            {!sold && (
              <div className="detail-calc">
                <span className="tl-head"><CalcIcon size={14} /> Hitung estimasi harga</span>
                <Calculator db={db} lockSheep={s} showCta={false} onResult={setRes} />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="sticky-bar">
          <div className="sticky-price"><span>{sold ? "Status" : "Estimasi"}</span><strong>{sold ? "Terjual" : idr(res.total)}</strong></div>
          {sold
            ? <button className="btn wa is-disabled" disabled><Check size={18} /> Sudah Terjual</button>
            : <a className="btn wa" href={waCalc(db.settings, res)} target="_blank" rel="noopener noreferrer"><WaIcon size={20} /> Pesan via WhatsApp</a>}
        </div>
      </div>
    </div>
  );
...
function Storefront({ db, onAdmin }) {
  const c = db.content, st = db.settings;
  ...
  const list = useMemo(() => db.sheep.filter(s =>
    (filter === "semua" || s.status === filter) && (jenis === "semua" || s.jenis === jenis) &&
    ...
    <div className="shop">
      <header className="shop-head">
        <div className="wrap head-in">
          <button className="brand" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}><span className="brand-mark"><Leaf size={18} /></span><span className="brand-name">{st.brand}</span></button>
          <nav className={"shop-nav" + (menu ? " open" : "")}>
            ...
            <button className="nav-admin" onClick={onAdmin}><LayoutDashboard size={15} /> Admin</button>
            <a className="nav-wa" href={`https://wa.me/${st.waNumber}`} target="_blank" rel="noopener noreferrer"><WaIcon size={16} /> Hubungi</a>
          </nav>
          <button className="icon-btn burger" onClick={() => setMenu(v => !v)}>{menu ? <X size={22} /> : <Menu size={22} />}</button>
        </div>
      </header>
      ...
      <section className="wrap hero">
        <div className="hero-copy rise">
          ...
          <div className="hero-cta">
            <button className="btn btn-primary" onClick={() => scroll("katalog")}>{c.hero.ctaLabel} <ArrowRight size={17} /></button>
            <button className="btn btn-ghost" onClick={() => scroll("kalkulator")}><CalcIcon size={17} /> Hitung Harga</button>
          </div>
          <div className="trust">{c.badges.map((b, i) => { const I = ICONS[b.ic] || Leaf; return <span key={i}><I size={15} /> {b.text}</span>; })}</div>
        </div>
        <div className="hero-video rise" style={{ animationDelay: "120ms" }}>
          <div className="pasture">
            <div className="sun" /><div className="cloud c1" /><div className="cloud c2" /><div className="hill h1" /><div className="hill h2" />
            <div className="graze"><SheepThumb tint="#5f7a6c" url={null} size={120} radius={0} playing /></div>
            <div className="graze g2"><SheepThumb tint="#6b8478" url={null} size={88} radius={0} playing /></div>
          </div>
          <span className="rec"><span className="rec-dot" /> Cuplikan kandang · loop</span>
        </div>
      </section>
      ...
      <section id="katalog" className="wrap section">
        <div className="sec-head-row">
          <div><span className="eyebrow">{list.length} ekor</span><h2 className="sec-title">Katalog Domba</h2></div>
          <div className="search-in"><Search size={16} /><input placeholder="Cari ID atau jenis…" value={q} onChange={e => setQ(e.target.value)} /></div>
        </div>
        <div className="filters">
          {["semua", "tersedia", "booked", "terjual"].map(f => <button key={f} className={"chip" + (filter === f ? " on" : "")} onClick={() => setFilter(f)}>{f === "semua" ? "Semua" : STATUS[f].label}</button>)}
          <select className="chip-sel" value={jenis} onChange={e => setJenis(e.target.value)}><option value="semua">Semua jenis</option>{db.breeds.map(b => <option key={b} value={b}>{b}</option>)}</select>
        </div>
        {list.length === 0
          ? <div className="empty"><Eye size={26} /><p>Tidak ada domba yang cocok. Ubah filter atau kata kunci.</p></div>
          : <div className="grid">{list.map((s, i) => <ProductCard key={s._k} s={s} onOpen={setSel} delay={i * 50} />)}</div>}
      </section>
      ...
      <section id="kalkulator" className="wrap section">
        <div className="calc-band">
          <div className="calc-intro">
            ...
            <p className="muted">Domba dijual <b>per kilogram</b>. Pilih domba atau masukkan harga sendiri, atur perkiraan berat, dan lihat estimasi totalnya secara langsung.</p>
            <ul className="calc-points">
              <li><Check size={15} /> Hitungan adil sesuai bobot hidup</li>
              <li><Check size={15} /> Estimasi DP otomatis</li>
              <li><Check size={15} /> Langsung kirim rincian ke WhatsApp</li>
            </ul>
          </div>
          <div className="calc-card"><Calculator db={db} /></div>
        </div>
      </section>

      {/* PROSES */}
      <section className="wrap section">
        <span className="eyebrow center">Mudah</span><h2 className="sec-title center">Cara pemesanan</h2>
        <div className="steps">
          {[[Search, "Pilih domba", "Telusuri katalog & lihat detail tiap domba."], [CalcIcon, "Hitung estimasi", "Pakai kalkulator untuk perkiraan harga per kg."], [WaIcon, "Konfirmasi WhatsApp", "Kirim rincian, sepakati harga & jadwal."], [Truck, "Timbang & antar", "Ditimbang ulang saat serah terima, lalu diantar."]].map((x, i) => {
            const I = x[0];
            return <div key={i} className="step"><span className="step-no">{i + 1}</span><span className="step-ic"><I size={18} /></span><b>{x[1]}</b><p>{x[2]}</p></div>;
          })}
        </div>
      </section>
      ...
      <section id="tentang" className="wrap section">
        <div className="about">
          <div className="about-art"><SheepThumb tint="#6F8B7C" url={null} size={"100%"} radius={18} /></div>
          <div className="about-copy">
            <span className="eyebrow">Tentang kami</span>
            <h2 className="sec-title">{c.about.title}</h2>
            <p>{c.about.body}</p>
            <div className="about-stats">
              <div><strong>{db.sheep.length}</strong><span>domba di katalog</span></div>
              <div><strong>{db.sheep.filter(s => s.status === "tersedia").length}</strong><span>siap dibeli</span></div>
              <div><strong>{db.breeds.length}</strong><span>jenis tersedia</span></div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* FAQ */}
      <section className="wrap section">
        <span className="eyebrow center">Pertanyaan umum</span><h2 className="sec-title center">FAQ</h2>
        <div className="faq">
          {[["Bagaimana cara harganya dihitung?", "Harga dihitung per kilogram bobot hidup. Estimasi di situs memakai bobot terakhir; berat final ditimbang ulang saat serah terima."], ["Apakah bisa diantar?", "Bisa. Pengiriman ke lokasi Anda diatur saat konfirmasi via WhatsApp, ongkos menyesuaikan jarak."], ["Apakah ada DP?", "Untuk mengamankan domba (status Booked), umumnya DP 30%. Pelunasan dilakukan saat serah terima."], ["Apakah domba sehat & bervaksin?", "Setiap domba diperiksa dokter hewan dengan riwayat pakan dan vaksin yang tercatat pada halaman detailnya."]].map((f, i) => <FaqItem key={i} q={f[0]} a={f[1]} />)}
        </div>
      </section>
      ...
      <footer id="kontak" className="shop-foot">
        <div className="wrap foot-in">
          <div>
            <div className="brand"><span className="brand-mark"><Leaf size={16} /></span><span className="brand-name">{st.brand}</span></div>
            <p className="foot-tag">{st.tagline}. Harga per kg, transparan, sehat & bervaksin.</p>
            <a className="btn btn-primary sm" href={`https://wa.me/${st.waNumber}`} target="_blank" rel="noopener noreferrer"><WaIcon size={16} /> Chat WhatsApp</a>
          </div>
          <div className="foot-meta">
            <span><MapPin size={14} /> {st.location}</span>
            <span><Clock size={14} /> {st.hours}</span>
            <a href={`https://wa.me/${st.waNumber}`} target="_blank" rel="noopener noreferrer"><Phone size={14} /> +{st.waNumber}</a>
            <a href={`mailto:${st.email}`}><Mail size={14} /> {st.email}</a>
          </div>
        </div>
        <div className="wrap foot-fine">© {new Date().getFullYear()} {st.brand} · Etalase domba</div>
      </footer>
...
function FaqItem({ q, a }) {
  ...
  return (
    <div className={"faq-item" + (open ? " open" : "")}>
      <button onClick={() => setOpen(o => !o)}><span>{q}</span><ChevronDown size={18} /></button>
      {open && <p>{a}</p>}
    </div>
  );
}
  ...
  return (
    <div className="login">
      <div className="login-card">
        <div className="brand brand-lg"><span className="brand-mark"><Leaf size={20} /></span><span className="brand-name">{brand}</span></div>
        <h1 className="login-title">Masuk ke Panel Admin</h1>
        <p className="login-sub">Kelola katalog, harga, konten, dan pengaturan etalase.</p>
        <Field label="Email"><input defaultValue="admin@lembahhijau.id" /></Field>
        <Field label="Kata sandi"><input type="password" placeholder="••••••••" /></Field>
        <button className="btn btn-primary btn-block" onClick={onLogin}>Masuk</button>
        <button className="link center-link" onClick={onBack}><ChevronLeft size={14} /> Kembali ke etalase</button>
        <p className="login-note"><AlertCircle size={13} /> Mode demo — kredensial apa pun diterima. Produksi: JWT, session timeout, rate-limit.</p>
      </div>
    </div>
  );
}
...
function Dashboard({ db, go }) {
  const c = useMemo(() => { const s = db.sheep; return { total: s.length, tersedia: s.filter(x => x.status === "tersedia").length, booked: s.filter(x => x.status === "booked").length, terjual: s.filter(x => x.status === "terjual").length, featured: s.filter(x => x.featured).length, nilai: s.filter(x => x.status !== "terjual").reduce((a, x) => a + est(x), 0) }; }, [db.sheep]);
  const statusData = [{ name: "Tersedia", value: c.tersedia, color: "#059669" }, { name: "Booked", value: c.booked, color: "#B97309" }, { name: "Terjual", value: c.terjual, color: "#8B948C" }].filter(d => d.value);
  const jenisData = useMemo(() => { const m = {}; db.sheep.forEach(s => m[s.jenis] = (m[s.jenis] || 0) + 1); return Object.entries(m).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value).slice(0, 6); }, [db.sheep]);
  const kpis = [["Total stok", c.total, <Package size={18} />, ""], ["Tersedia", c.tersedia, <CheckCircle2 size={18} />, "k-green"], ["Booked", c.booked, <Clock size={18} />, "k-amber"], ["Terjual", c.terjual, <TrendingUp size={18} />, "k-gray"]];
  return (
    <div className="page">
      <div className="kpis">
        {kpis.map(x => <div key={x[0]} className={"kpi " + x[3]}><span className="kpi-ic">{x[2]}</span><div><strong>{x[1]}</strong><span>{x[0]}</span></div></div>)}
        <div className="kpi k-wide"><span className="kpi-ic"><Wallet size={18} /></span><div><strong>{idr(c.nilai)}</strong><span>Estimasi nilai inventaris</span></div></div>
      </div>
      <div className="grid-2">
        <div className="card"><div className="card-head"><h3>Distribusi status</h3></div>
          <div className="chart-wrap"><ResponsiveContainer width="100%" height={220}><PieChart><Pie data={statusData} dataKey="value" nameKey="name" innerRadius={58} outerRadius={84} paddingAngle={2} stroke="none">{statusData.map((d, i) => <Cell key={i} fill={d.color} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer><div className="donut-center"><strong>{c.total}</strong><span>domba</span></div></div>
          <div className="legend">{statusData.map(d => <span key={d.name}><i style={{ background: d.color }} />{d.name} · {d.value}</span>)}</div>
        </div>
        <div className="card"><div className="card-head"><h3>Stok per jenis</h3></div>
          <ResponsiveContainer width="100%" height={262}><BarChart data={jenisData} layout="vertical" margin={{ left: 8, right: 16 }}><XAxis type="number" hide /><YAxis type="category" dataKey="name" width={120} tick={{ fontSize: 12, fill: "#5B6B62" }} axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: "rgba(5,150,105,.06)" }} /><Bar dataKey="value" fill="#059669" radius={[0, 6, 6, 0]} barSize={16} /></BarChart></ResponsiveContainer>
        </div>
      </div>
      <div className="grid-2">
        <div className="card"><div className="card-head"><h3>Aktivitas terbaru</h3><button className="link" onClick={() => go("log")}>Lihat semua <ChevronRight size={14} /></button></div>
          <div className="feed">{db.log.slice(0, 6).map(l => <div key={l.id} className="feed-row"><span className={"feed-dot " + l.kind} /><div><p>{l.action}</p><span>{fmtTime(l.at)} · {l.who}</span></div></div>)}</div>
        </div>
        <div className="card card-cta"><h3>Tindakan cepat</h3>
          <div className="qa">
            <button className="qa-btn" onClick={() => go("inventory:new")}><Plus size={16} /> Tambah domba</button>
            <button className="qa-btn" onClick={() => go("content")}><FileText size={16} /> Edit konten</button>
            <button className="qa-btn" onClick={() => go("etalase")}><Eye size={16} /> Lihat etalase</button>
            <button className="qa-btn" onClick={() => go("settings")}><Settings size={16} /> Pengaturan</button>
          </div>
          <div className="hint-box"><Sparkles size={15} /> {c.featured} domba ditandai <b>unggulan</b> & tampil menonjol.</div>
        </div>
      </div>
    </div>
  );
...
function SheepEditor({ initial, breeds, onSave, onClose, toast }) {
  ...
  const cover = s.media.find(m => m.cover) || s.media[0];
  const addMedia = (file) => { if (!file) return; const type = file.type.startsWith("video") ? "klip" : "foto"; const url = URL.createObjectURL(file); setS(p => ({ ...p, media: [...p.media, { id: uid("m_"), type, name: file.name, url, cover: p.media.length === 0 }] })); };
  const setCover = (id) => setS(p => ({ ...p, media: p.media.map(m => ({ ...m, cover: m.id === id })) }));
  const moveMedia = (i, d) => setS(p => { const a = [...p.media]; const j = i + d; if (j < 0 || j >= a.length) return p; [a[i], a[j]] = [a[j], a[i]]; return { ...p, media: a }; });
  const delMedia = (id) => setS(p => { const a = p.media.filter(m => m.id !== id); if (a.length && !a.some(m => m.cover)) a[0].cover = true; return { ...p, media: a }; });
  const addLog = () => setS(p => ({ ...p, log: [...p.log, { id: uid("l_"), t: "", x: "" }] }));
  const setLog = (id, k, v) => setS(p => ({ ...p, log: p.log.map(l => l.id === id ? { ...l, [k]: v } : l) }));
  const delLog = (id) => setS(p => ({ ...p, log: p.log.filter(l => l.id !== id) }));
  const submit = () => { if (!s.id.trim()) return toast("ID domba wajib diisi", "err"); if (!s.jenis.trim()) return toast("Jenis wajib diisi", "err"); onSave({ ...s, bobot: Number(s.bobot) || 0, umur: Number(s.umur) || 0, hargaPerKg: Number(s.hargaPerKg) || 0 }); };
  ...
  return (
    <div className="drawer-overlay">
      ...
      <div className="drawer">
        <div className="drawer-head"><div><span className="eyebrow">{initial._isNew ? "Domba baru" : "Edit domba"}</span><h2>{s.id || "Tanpa ID"}</h2></div><button className="icon-btn" onClick={onClose}><X size={20} /></button></div>
        <div className="drawer-body">
          <div className="editor-grid">
            <div className="editor-main">
              <section className="form-sec"><h4>Informasi dasar</h4>
                <div className="row-2"><Field label="ID Domba" required hint="Unik, mis. DMB-008"><input value={s.id} onChange={e => set("id", e.target.value)} placeholder="DMB-008" /></Field>
                  <Field label="Jenis" required><input list="brds" value={s.jenis} onChange={e => set("jenis", e.target.value)} placeholder="Pilih / ketik" /><datalist id="brds">{breeds.map(b => <option key={b} value={b} />)}</datalist></Field></div>
                <div className="row-3"><Field label="Bobot (kg)"><input type="number" value={s.bobot} onChange={e => set("bobot", e.target.value)} /></Field>
                  <Field label="Umur (bln)"><input type="number" value={s.umur} onChange={e => set("umur", e.target.value)} /></Field>
                  <Field label="Harga / kg (Rp)" required><input type="number" value={s.hargaPerKg} onChange={e => set("hargaPerKg", e.target.value)} /></Field></div>
                <div className="est-line"><CalcIcon size={14} /> Estimasi total: <b>{idr(estTotal)}</b> <span className="muted">({idr(s.hargaPerKg || 0)}/kg × {s.bobot || 0} kg)</span></div>
                <div className="row-2"><Field label="Warna / corak"><input value={s.warna} onChange={e => set("warna", e.target.value)} /></Field>
                  <Field label="Status"><select value={s.status} onChange={e => set("status", e.target.value)}>{Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></Field></div>
                <Toggle on={s.featured} onChange={v => set("featured", v)} label="Tampilkan sebagai domba unggulan" />
              </section>
              <section className="form-sec"><h4>Deskripsi & perawatan</h4>
                <Field label="Deskripsi"><textarea rows={3} value={s.desc} onChange={e => set("desc", e.target.value)} /></Field>
                <div className="row-2"><Field label="Pakan"><input value={s.pakan} onChange={e => set("pakan", e.target.value)} /></Field><Field label="Vaksin & kesehatan"><input value={s.vaksin} onChange={e => set("vaksin", e.target.value)} /></Field></div>
              </section>
              <section className="form-sec"><div className="sec-head"><h4>Media (foto & klip)</h4><span className="auto-chip"><Sparkles size={12} /> dikompres otomatis</span></div>
                <div className="media-grid">
                  {s.media.map((m, i) => <div key={m.id} className={"media-cell" + (m.cover ? " is-cover" : "")}><SheepThumb tint={s.tint} url={m.url} kind={m.type} size={"100%"} radius={10} /><span className="media-type">{m.type === "klip" ? <Video size={11} /> : <ImageIcon size={11} />}{m.type}</span>{m.cover && <span className="media-cover"><Star size={11} fill="currentColor" /> sampul</span>}<div className="media-ops"><button onClick={() => setCover(m.id)}><Star size={13} /></button><button onClick={() => moveMedia(i, -1)}><ArrowUp size={13} /></button><button onClick={() => moveMedia(i, 1)}><ArrowDown size={13} /></button><button onClick={() => delMedia(m.id)}><Trash2 size={13} /></button></div></div>)}
                  <button className="media-add" onClick={() => fileRef.current?.click()}><Upload size={18} /><span>Unggah foto / video</span></button>
                  <input ref={fileRef} type="file" accept="image/*,video/*" hidden onChange={e => { addMedia(e.target.files?.[0]); e.target.value = ""; }} />
                </div>
                <p className="field-hint">Klip vertikal 3–5 detik diputar otomatis tanpa suara di kartu produk.</p>
              </section>
              <section className="form-sec"><div className="sec-head"><h4>Riwayat perawatan</h4><button className="link" onClick={addLog}><Plus size={14} /> Tambah</button></div>
                {s.log.length === 0 && <p className="field-hint">Belum ada catatan.</p>}
                {s.log.map(l => <div key={l.id} className="log-edit"><input className="log-t" placeholder="Tgl" value={l.t} onChange={e => setLog(l.id, "t", e.target.value)} /><input className="log-x" placeholder="Catatan" value={l.x} onChange={e => setLog(l.id, "x", e.target.value)} /><button className="icon-btn sm" onClick={() => delLog(l.id)}><Trash2 size={14} /></button></div>)}
              </section>
            </div>
            <aside className="editor-aside"><span className="eyebrow">Pratinjau kartu</span>
              <div className="prev-card"><div className="prev-media"><SheepThumb tint={s.tint} url={cover?.url} kind={cover?.type} size={"100%"} radius={0} /><span className={"pill float " + STATUS[s.status].cls}>{STATUS[s.status].label}</span></div>
                <div className="prev-body"><div className="prod-top"><span className="mono">{s.id || "DMB-???"}</span><span className="muted sm">{s.warna || "—"}</span></div><h3>{s.jenis || "Jenis"}</h3><div className="prod-meta"><span><Scale size={13} /> {s.bobot || 0} kg</span></div><PriceTag s={s} /></div></div>
              <div className="tints">{TINTS.map(t => <button key={t} className={"tint-dot" + (s.tint === t ? " is-on" : "")} style={{ background: t }} onClick={() => set("tint", t)} />)}</div>
              <p className="field-hint">Warna placeholder dipakai bila foto belum diunggah.</p>
            </aside>
          </div>
        </div>
        <div className="drawer-foot"><button className="btn btn-ghost" onClick={onClose}>Batal</button><button className="btn btn-primary" onClick={submit}><Save size={16} /> Simpan domba</button></div>
      </div>
    </div>
  );
}
...
function ConfirmDialog({ message, onConfirm, onCancel }) {
  return (<div className="drawer-overlay center"><div className="scrim" onClick={onCancel} /><div className="confirm"><span className="confirm-ic"><AlertCircle size={22} /></span><p>{message}</p><div className="confirm-act"><button className="btn btn-ghost" onClick={onCancel}>Batal</button><button className="btn btn-danger" onClick={onConfirm}>Hapus</button></div></div></div>);
}

function Inventory({ db, actions, toast, openNew, onConsumedNew }) {
  const [q, setQ] = useState(""); const [fS, setFS] = useState("semua"); const [fJ, setFJ] = useState("semua"); const [sort, setSort] = useState("baru");
  const [sel, setSel] = useState([]); const [editing, setEditing] = useState(null); const [confirm, setConfirm] = useState(null);
  const blank = () => ({ _k: uid(), _isNew: true, id: "", jenis: db.breeds[0] || "", bobot: "", umur: "", status: "tersedia", hargaPerKg: 110000, warna: "", featured: false, desc: "", pakan: "", vaksin: "", tint: TINTS[Math.floor(Math.random() * TINTS.length)], media: [], log: [], createdAt: now(), updatedAt: now() });
  useEffect(() => { if (openNew) { setEditing(blank()); onConsumedNew(); } }, [openNew]); // eslint-disable-line
  const dup = (s) => setEditing({ ...s, _k: uid(), _isNew: true, id: s.id + "-COPY", media: s.media.map(m => ({ ...m, id: uid("m_") })), log: s.log.map(l => ({ ...l, id: uid("l_") })) });
  const rows = useMemo(() => { let r = db.sheep.filter(s => (fS === "semua" || s.status === fS) && (fJ === "semua" || s.jenis === fJ) && (!q || (s.id + s.jenis).toLowerCase().includes(q.toLowerCase()))); return [...r].sort((a, b) => sort === "bobot" ? b.bobot - a.bobot : sort === "harga" ? b.hargaPerKg - a.hargaPerKg : sort === "id" ? a.id.localeCompare(b.id) : new Date(b.updatedAt) - new Date(a.updatedAt)); }, [db.sheep, q, fS, fJ, sort]);
  const allSel = rows.length > 0 && rows.every(r => sel.includes(r._k));
  const toggle = (k) => setSel(p => p.includes(k) ? p.filter(x => x !== k) : [...p, k]);
  const exportCSV = (l) => { const h = ["ID", "Jenis", "Bobot", "Umur", "HargaPerKg", "EstimasiTotal", "Status"]; const lines = [h.join(",")].concat(l.map(s => [s.id, s.jenis, s.bobot, s.umur, s.hargaPerKg, est(s), s.status].join(","))); const blob = new Blob(["\ufeff" + lines.join("\n")], { type: "text/csv;charset=utf-8" }); const u = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = u; a.download = "katalog-domba.csv"; a.click(); URL.revokeObjectURL(u); toast("CSV diunduh"); };
  return (
    <div className="page">
      <div className="toolbar">
        <div className="search-in"><Search size={16} /><input placeholder="Cari ID atau jenis…" value={q} onChange={e => setQ(e.target.value)} /></div>
        <select value={fS} onChange={e => setFS(e.target.value)}><option value="semua">Semua status</option>{Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select>
        <select value={fJ} onChange={e => setFJ(e.target.value)}><option value="semua">Semua jenis</option>{db.breeds.map(b => <option key={b} value={b}>{b}</option>)}</select>
        <select value={sort} onChange={e => setSort(e.target.value)}><option value="baru">Terbaru</option><option value="bobot">Bobot ↓</option><option value="harga">Harga/kg ↓</option><option value="id">ID A–Z</option></select>
        <div className="tb-spacer" /><button className="btn btn-ghost" onClick={() => exportCSV(rows)}><Download size={15} /> Export</button><button className="btn btn-primary" onClick={() => setEditing(blank())}><Plus size={16} /> Tambah Domba</button>
      </div>
      {sel.length > 0 && <div className="bulkbar"><span>{sel.length} dipilih</span><select defaultValue="" onChange={e => { if (e.target.value) { actions.bulkStatus(sel, e.target.value); setSel([]); e.target.value = ""; } }}><option value="">Ubah status…</option>{Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select><button className="btn btn-danger sm" onClick={() => setConfirm({ msg: `Hapus ${sel.length} domba?`, fn: () => { actions.bulkDelete(sel); setSel([]); } })}><Trash2 size={14} /> Hapus</button><button className="link" onClick={() => setSel([])}>Batal</button></div>}
      <div className="table-card"><table className="table">
        <thead><tr><th className="c-check"><input type="checkbox" checked={allSel} onChange={() => setSel(allSel ? [] : rows.map(r => r._k))} /></th><th>Domba</th><th>Bobot</th><th>Harga/kg</th><th>Estimasi</th><th>Status</th><th className="c-act"></th></tr></thead>
        <tbody>{rows.map(s => { const cover = s.media.find(m => m.cover) || s.media[0]; return (
          <tr key={s._k} className={sel.includes(s._k) ? "is-sel" : ""}>
            <td className="c-check"><input type="checkbox" checked={sel.includes(s._k)} onChange={() => toggle(s._k)} /></td>
            <td><div className="cell-sheep"><SheepThumb tint={s.tint} url={cover?.url} kind={cover?.type} /><div><b>{s.jenis} {s.featured && <Star size={12} fill="#B97309" color="#B97309" />}</b><span className="mono">{s.id}</span></div></div></td>
            <td>{s.bobot} kg</td><td>{idr(s.hargaPerKg)}</td><td className="muted">{idrShort(est(s))}</td>
            <td><select className={"status-sel " + STATUS[s.status].cls} value={s.status} onChange={e => actions.setStatus(s._k, e.target.value)}>{Object.entries(STATUS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></td>
            <td className="c-act"><button className="icon-btn sm" onClick={() => setEditing({ ...s })}><Pencil size={15} /></button><button className="icon-btn sm" onClick={() => dup(s)}><Copy size={15} /></button><button className="icon-btn sm danger" onClick={() => setConfirm({ msg: `Hapus ${s.id}?`, fn: () => actions.delSheep(s._k) })}><Trash2 size={15} /></button></td>
          </tr>); })}
          {rows.length === 0 && <tr><td colSpan={7} className="empty-row"><Eye size={22} /><p>Tidak ada domba yang cocok.</p></td></tr>}
        </tbody></table></div>
      <p className="table-foot">Menampilkan {rows.length} dari {db.sheep.length} domba</p>
      {editing && <SheepEditor initial={editing} breeds={db.breeds} toast={toast} onClose={() => setEditing(null)} onSave={(s) => { if (actions.saveSheep(s)) { setEditing(null); toast(s._isNew ? "Domba ditambahkan" : "Perubahan disimpan"); } }} />}
      {confirm && <ConfirmDialog message={confirm.msg} onCancel={() => setConfirm(null)} onConfirm={() => { confirm.fn(); setConfirm(null); }} />}
    </div>
  );
}
...
function ContentEditor({ db, save, toast }) {
  ...
  const setAbout = (k, v) => setC(p => ({ ...p, about: { ...p.about, [k]: v } }));
  const setBadge = (i, k, v) => setC(p => ({ ...p, badges: p.badges.map((b, j) => j === i ? { ...b, [k]: v } : b) }));
  return (
    <div className="page">
      <div className="grid-2 top">
        <div className="card"><div className="card-head"><h3>Hero / banner</h3></div>
          <Field label="Eyebrow"><input value={c.hero.eyebrow} onChange={e => setHero("eyebrow", e.target.value)} /></Field>
          <Field label="Judul utama" hint="Gunakan Enter untuk pindah baris"><textarea rows={2} value={c.hero.title} onChange={e => setHero("title", e.target.value)} /></Field>
          <Field label="Sub-judul"><textarea rows={3} value={c.hero.sub} onChange={e => setHero("sub", e.target.value)} /></Field>
          <Field label="Teks tombol CTA"><input value={c.hero.ctaLabel} onChange={e => setHero("ctaLabel", e.target.value)} /></Field>
        </div>
        <div className="card"><div className="card-head"><h3>Pratinjau hero</h3></div>
          <div className="hero-prev"><span className="eyebrow">{c.hero.eyebrow}</span><h2>{c.hero.title}</h2><p>{c.hero.sub}</p><span className="prev-cta">{c.hero.ctaLabel || "Tombol"}</span></div>
        </div>
      </div>
      <div className="card"><div className="card-head"><h3>Badge kepercayaan</h3><button className="link" onClick={() => setC(p => ({ ...p, badges: [...p.badges, { ic: "leaf", text: "Keunggulan baru" }] }))}><Plus size={14} /> Tambah</button></div>
        <div className="badge-list">{c.badges.map((b, i) => <div key={i} className="badge-edit"><select value={b.ic} onChange={e => setBadge(i, "ic", e.target.value)}>{Object.keys(ICONS).map(k => <option key={k} value={k}>{k}</option>)}</select><input value={b.text} onChange={e => setBadge(i, "text", e.target.value)} /><button className="icon-btn sm danger" onClick={() => setC(p => ({ ...p, badges: p.badges.filter((_, j) => j !== i) }))}><Trash2 size={14} /></button></div>)}</div>
      </div>
      <div className="card"><div className="card-head"><h3>Tentang peternakan</h3></div><Field label="Judul"><input value={c.about.title} onChange={e => setAbout("title", e.target.value)} /></Field><Field label="Isi"><textarea rows={4} value={c.about.body} onChange={e => setAbout("body", e.target.value)} /></Field></div>
      <div className="save-bar"><button className="btn btn-primary" onClick={() => { save(c); toast("Konten halaman disimpan"); }}><Save size={16} /> Simpan konten</button></div>
    </div>
  );
}
...
function Breeds({ db, save, toast }) {
  ...
  const counts = useMemo(() => { const m = {}; db.sheep.forEach(s => m[s.jenis] = (m[s.jenis] || 0) + 1); return m; }, [db.sheep]);
  const add = () => { const v = val.trim(); if (!v) return; if (db.breeds.some(b => b.toLowerCase() === v.toLowerCase())) return toast("Jenis sudah ada", "err"); save([...db.breeds, v]); setVal(""); toast("Jenis ditambahkan"); };
  const del = (b) => { if (counts[b]) return toast(`Masih dipakai ${counts[b]} domba`, "err"); save(db.breeds.filter(x => x !== b)); toast("Jenis dihapus"); };
  return (<div className="page"><div className="card narrow"><div className="card-head"><h3>Jenis / ras domba</h3></div>
    <div className="add-row"><input placeholder="Nama jenis baru…" value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => e.key === "Enter" && add()} /><button className="btn btn-primary" onClick={add}><Plus size={16} /> Tambah</button></div>
    <div className="breed-list">{db.breeds.map(b => <div key={b} className="breed-row"><span className="breed-ic"><Tag size={15} /></span><b>{b}</b><span className="breed-count">{counts[b] || 0} domba</span><button className="icon-btn sm danger" onClick={() => del(b)}><Trash2 size={14} /></button></div>)}</div>
  </div></div>);
}
...
function SettingsPage({ db, save, onReset, toast, persist }) {
  ...
  return (
    <div className="page">
      <div className="grid-2 top">
        <div className="card"><div className="card-head"><h3>Identitas peternakan</h3></div><Field label="Nama brand"><input value={s.brand} onChange={e => set("brand", e.target.value)} /></Field><Field label="Tagline"><input value={s.tagline} onChange={e => set("tagline", e.target.value)} /></Field><Field label="Lokasi"><input value={s.location} onChange={e => set("location", e.target.value)} /></Field><Field label="Jam operasional"><input value={s.hours} onChange={e => set("hours", e.target.value)} /></Field></div>
        <div className="card"><div className="card-head"><h3>Kontak & WhatsApp</h3></div>
          <Field label="Nomor WhatsApp" hint="Format internasional tanpa +, mis. 6281234567890"><div className="wa-test"><input value={s.waNumber} onChange={e => set("waNumber", e.target.value)} /><a className="btn btn-ghost sm" href={`https://wa.me/${s.waNumber}`} target="_blank" rel="noopener noreferrer"><Phone size={14} /> Tes</a></div></Field>
          <Field label="Email"><input value={s.email} onChange={e => set("email", e.target.value)} /></Field>
          <Field label="URL situs" hint="Dipakai untuk link pada pesan WhatsApp"><input value={s.siteUrl} onChange={e => set("siteUrl", e.target.value)} /></Field>
          <Field label="Warna aksen"><div className="color-row"><input type="color" value={s.accent} onChange={e => set("accent", e.target.value)} /><input value={s.accent} onChange={e => set("accent", e.target.value)} /></div></Field>
        </div>
      </div>
      <div className="card"><div className="card-head"><h3>Data & sistem</h3></div>
        <div className="sys-row"><div><b>Status penyimpanan</b><p className="muted">{persist ? "Aktif — perubahan tersimpan & menetap antar sesi." : "Sementara (memori) — perubahan hilang saat ditutup."}</p></div><span className={"sys-chip " + (persist ? "ok" : "warn")}>{persist ? <Check size={13} /> : <AlertCircle size={13} />}{persist ? "Tersimpan" : "Memori"}</span></div>
        <div className="sys-row"><div><b>Reset data contoh</b><p className="muted">Mengembalikan seluruh data ke kondisi awal.</p></div><button className="btn btn-ghost sm" onClick={onReset}><RotateCcw size={14} /> Reset</button></div>
        <div className="hint-box"><ShieldCheck size={15} /> Produksi: JWT + session timeout, security headers (CSP/HSTS), sanitasi unggahan, backup DB di server.</div>
      </div>
      <div className="save-bar"><button className="btn btn-primary" onClick={() => { save(s); toast("Pengaturan disimpan"); }}><Save size={16} /> Simpan pengaturan</button></div>
    </div>
  );
}
...
function ActivityLog({ db }) {
  ...
  const kinds = { create: "Tambah", update: "Ubah", delete: "Hapus", status: "Status", content: "Konten", settings: "Pengaturan" };
  const rows = db.log.filter(l => f === "semua" || l.kind === f);
  return (<div className="page"><div className="toolbar"><select value={f} onChange={e => setF(e.target.value)}><option value="semua">Semua tipe</option>{Object.entries(kinds).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select></div>
    <div className="table-card"><table className="table"><thead><tr><th>Waktu</th><th>Pengguna</th><th>Tipe</th><th>Aktivitas</th></tr></thead>
      <tbody>{rows.map(l => <tr key={l.id}><td className="muted">{fmtTime(l.at)}</td><td><b>{l.who}</b></td><td><span className={"kind-chip " + l.kind}>{kinds[l.kind] || l.kind}</span></td><td>{l.action}</td></tr>)}
        {rows.length === 0 && <tr><td colSpan={4} className="empty-row"><ClipboardList size={22} /><p>Belum ada aktivitas.</p></td></tr>}</tbody></table></div></div>);
}

const NAV = [
  { group: "Utama", items: [["dashboard", "Dashboard", LayoutDashboard], ["inventory", "Katalog Domba", Package]] },
  { group: "Konten", items: [["content", "Konten Halaman", FileText], ["breeds", "Jenis Domba", Tag]] },
  { group: "Sistem", items: [["log", "Log Aktivitas", ClipboardList], ["settings", "Pengaturan", Settings]] },
];
const TITLES = { dashboard: "Dashboard", inventory: "Katalog Domba", content: "Konten Halaman", breeds: "Jenis Domba", log: "Log Aktivitas", settings: "Pengaturan" };

function AdminApp({ db, actions, persist, toast, onExit }) {
  const [route, setRoute] = useState("dashboard");
  const [sidebar, setSidebar] = useState(false);
  const [openNew, setOpenNew] = useState(false);
  const go = (r) => { if (r === "etalase") return onExit(); if (r === "inventory:new") { setRoute("inventory"); setOpenNew(true); } else setRoute(r); setSidebar(false); };
  return (
    <div className="cms-app">
      <aside className={"sidebar" + (sidebar ? " open" : "")}>
        <div className="brand"><span className="brand-mark"><Leaf size={18} /></span><span className="brand-name">{db.settings.brand}</span></div>
        <nav className="side-nav">{NAV.map(g => <div key={g.group} className="nav-group"><span className="nav-group-t">{g.group}</span>{g.items.map(([k, l, I]) => <button key={k} className={"nav-item" + (route === k ? " is-active" : "")} onClick={() => go(k)}><I size={17} /> {l}</button>)}</div>)}</nav>
        <div className="side-foot"><div className="side-acc"><span className="avatar">A</span><div><b>Admin</b><span>Akses penuh</span></div></div><button className="nav-item" onClick={onExit}><Store size={16} /> Lihat etalase</button></div>
      </aside>
      {sidebar && <div className="side-scrim" onClick={() => setSidebar(false)} />}
      <div className="main">
        <header className="topbar"><button className="icon-btn only-mobile" onClick={() => setSidebar(true)}><Menu size={20} /></button><h1 className="top-title">{TITLES[route]}</h1><div className="tb-spacer" /><button className="btn btn-ghost sm only-desk" onClick={onExit}><ExternalLink size={14} /> Lihat etalase</button><button className="icon-btn"><Bell size={18} /></button><span className="avatar">A</span></header>
        <main className="content">
          {route === "dashboard" && <Dashboard db={db} go={go} />}
          {route === "inventory" && <Inventory db={db} actions={actions} toast={toast} openNew={openNew} onConsumedNew={() => setOpenNew(false)} />}
          {route === "content" && <ContentEditor db={db} save={actions.saveContent} toast={toast} />}
          {route === "breeds" && <Breeds db={db} save={actions.saveBreeds} toast={toast} />}
          {route === "log" && <ActivityLog db={db} />}
          {route === "settings" && <SettingsPage db={db} save={actions.saveSettings} onReset={actions.reset} toast={toast} persist={persist} />}
        </main>
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    const id = "etl-fonts"; if (!document.getElementById(id)) { const l = document.createElement("link"); l.id = id; l.rel = "stylesheet"; l.href = "https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,500;9..144,600&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap"; document.head.appendChild(l); }
  }, []);

  const toast = useCallback((msg, type = "ok") => { const id = uid(); setToasts(t => [...t, { id, msg, type }]); setTimeout(() => setToasts(t => t.filter(x => x.id !== id)), 2600); }, []);
  const logE = (action, kind) => ({ id: uid(), at: now(), who: "Admin", action, kind });
  const actions = useMemo(() => ({
    saveSheep: (s) => { if (db.sheep.some(x => x.id.toLowerCase() === s.id.toLowerCase() && x._k !== s._k)) { toast("ID sudah dipakai", "err"); return false; } const ex = db.sheep.some(x => x._k === s._k); const cl = { ...s }; delete cl._isNew; const sheep = ex ? db.sheep.map(x => x._k === s._k ? { ...cl, updatedAt: now() } : x) : [{ ...cl, createdAt: now(), updatedAt: now() }, ...db.sheep]; commit({ ...db, sheep, log: [logE(`${ex ? "Memperbarui" : "Menambah"} ${s.id}`, ex ? "update" : "create"), ...db.log].slice(0, 200) }); return true; },
    delSheep: (k) => { const s = db.sheep.find(x => x._k === k); commit({ ...db, sheep: db.sheep.filter(x => x._k !== k), log: [logE(`Menghapus ${s?.id}`, "delete"), ...db.log].slice(0, 200) }); toast("Domba dihapus"); },
    setStatus: (k, status) => { const s = db.sheep.find(x => x._k === k); commit({ ...db, sheep: db.sheep.map(x => x._k === k ? { ...x, status, updatedAt: now() } : x), log: [logE(`${s?.id} → ${STATUS[status].label}`, "status"), ...db.log].slice(0, 200) }); },
    bulkStatus: (keys, status) => { commit({ ...db, sheep: db.sheep.map(x => keys.includes(x._k) ? { ...x, status, updatedAt: now() } : x), log: [logE(`Ubah status ${keys.length} domba → ${STATUS[status].label}`, "status"), ...db.log].slice(0, 200) }); toast("Status diperbarui"); },
    bulkDelete: (keys) => { commit({ ...db, sheep: db.sheep.filter(x => !keys.includes(x._k)), log: [logE(`Menghapus ${keys.length} domba`, "delete"), ...db.log].slice(0, 200) }); toast("Domba dihapus"); },
    saveContent: (content) => commit({ ...db, content, log: [logE("Memperbarui konten halaman", "content"), ...db.log].slice(0, 200) }),
    saveBreeds: (breeds) => commit({ ...db, breeds, log: [logE("Memperbarui daftar jenis", "content"), ...db.log].slice(0, 200) }),
    saveSettings: (settings) => commit({ ...db, settings, log: [logE("Memperbarui pengaturan", "settings"), ...db.log].slice(0, 200) }),
    reset: () => { commit(makeSeed()); toast("Data direset"); },
  }), [db, commit, toast]);

  if (!db) return <div className="boot"><div className="boot-dot" /> Memuat…</div>;

  return (
    <div className="app" style={{ "--accent": db.settings.accent || "#059669" }}>
      <style>{CSS}</style>
      {mode === "shop"
        ? <Storefront db={db} onAdmin={() => setMode("admin")} />
        : (authed
          ? <AdminApp db={db} actions={actions} persist={persist} toast={toast} onExit={() => setMode("shop")} />
          : <Login brand={db.settings.brand} onLogin={() => setAuthed(true)} onBack={() => setMode("shop")} />)}
      <div className="toasts">{toasts.map(t => <div key={t.id} className={"toast " + t.type}>{t.type === "err" ? <AlertCircle size={16} /> : <CheckCircle2 size={16} />}{t.msg}</div>)}</div>
    </div>
  );
}
